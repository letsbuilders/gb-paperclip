...
describe Paperclip::Storage::S3V2 do
  before do
    AWS.stub!
  end
  ...
  context ':bucket option via :s3_credentials' do
    ...
    before do
      rebuild_model storage: :s3_v2, s3_credentials: { bucket: 'testing' }
      @dummy = Dummy.new
    end
  ...
  context ':bucket option' do
    ...
    before do
      rebuild_model storage: :s3_v2, bucket: 'testing', s3_credentials: {}
      @dummy = Dummy.new
    end
  ...
  context 's3_protocol' do
    ['http', :http, ''].each do |protocol|
      context "as #{protocol.inspect}" do
        before do
          rebuild_model storage: :s3_v2, s3_protocol: protocol
  ...
  context 's3_host_name' do
    ...
    it 'uses the S3 bucket with the correct host name' do
      assert_equal 'ap-northeast-1', @dummy.avatar.s3_region
    end
  end
  ...
  context 'An attachment that uses S3 for storage and has a proc for styles' do
    before do
      rebuild_model styles:           lambda { |attachment| attachment.instance.counter; { thumbnail: { geometry: '50x50#', s3_headers: { 'Cache-Control' => 'max-age=31557600' } } } },
                    ...
                    s3_region:        'us-west-1',
                    s3_storage_class: { thumbnail: :reduced_redundancy }
      ...
      @dummy.avatar.stubs(:s3_object).with(:thumbnail).returns(thumb_object)
      #object.expects(:upload_file).with(anything, anything).twice
      object.expects(:upload_file).with(anything, content_type: 'image/png', acl: 'public-read')
      thumb_object.expects(:upload_file).with(anything, content_type: 'image/png', acl: 'public-read', storage_class: 'REDUCED_REDUNDANCY', cache_control: 'max-age=31557600')
      @dummy.save
    end
  ...
  context '' do
    ...
    it 'returns a url based on an S3 subdomain' do
      expect(@dummy.avatar.url).to match %r{^http://bucket.s3.amazonaws.com/avatars/data[^\.]}
    end
  end
  ...
  context '' do
    ...
    it 'returns a url based on the host_alias' do
      expect(@dummy.avatar.url).to match %r{^https://something.something.com/avatars/data[^\.]}
    end
  end
  ...
  context 'generating a url with a proc as the host alias' do
    ...
    it 'returns a url based on the host_alias' do
      expect(@dummy.avatar.url).to match %r{^http://cdn1.example.com/avatars/data[^\.]}
      expect(@dummy.avatar.url).to match %r{^http://cdn2.example.com/avatars/data[^\.]}
    end

    it 'still returns the bucket name' do
      expect(@dummy.avatar.bucket_name).to eq 'prod_bucket'
    end
  ...
  context 'Generating a secure url with an expiration' do
    ...
    it 'uses default options' do
      ...
      rails_env('production') do
        ...
        object.expects(:presigned_url).with(:get, expires_in: 3600, secure: true)
    ...
    it 'allows overriding s3_url_options' do
      ...
      rails_env('production') do
        ...
        object = stub
        @dummy.avatar.stubs(:s3_object).returns(object)
        object.expects(:presigned_url).with(:get, expires_in: 3600, secure: true, response_content_disposition: 'inline')
    ...
    it 'allows overriding s3_object options with a proc' do
      ...
      rails_env('production') do
        ...
        object = stub
        @dummy.avatar.stubs(:s3_object).returns(object)
        object.expects(:presigned_url).with(:get, expires_in: 3600, secure: true, response_content_type: 'image/png')
  ...
  context 'Generating a url with an expiration for each style' do
    ...
    it 'generates a url for the thumb' do
      ...
      object.expects(:presigned_url).with(:get, expires_in: 1800, secure: true)
  ...
  context 'Parsing S3 credentials with a s3_host_name in them' do
    ...
    it 'gets the right s3_host_name in production' do
      rails_env('production') do
        ...
        expect(@dummy.avatar.s3_region).to eq 'world-end'
      end
    end
    ...
    it 'gets the right s3_host_name in development' do
      rails_env('development') do
        ...
        expect(@dummy.avatar.s3_region).to eq 'ap-northeast-1'
      end
    end
  ...
  context 'An attachment with S3 storage' do
    ...
    it 'is extended by the S3 module' do
      assert Dummy.new.avatar.is_a?(Paperclip::Storage::S3V2)
    end
    ...
    context 'when assigned' do
      ...
      context 'and saved' do
        before do
          object = stub
          @dummy.avatar.stubs(:s3_object).returns(object)
          object.expects(:upload_file).with(anything,
      ...
      context 'and saved without a bucket' do
        ...
        it 'succeeds' do
          # AWS::S3::Client.any_instance.expects(:create_bucket).with(bucket: 'testing')
          # object = stub
          # @dummy.avatar.stubs(:s3_object).returns(object)
          # object.stubs(:upload_file).
          #     raises(Aws::S3::Errors::NoSuchBucket.new(stub,
          #                                              stub(status: 404,
          #                                                   body:   '<foo/>'))).
          #     then.returns(nil)
          # @dummy.save
          pending 'need proper stub'
          assert false
        end
      end
  ...
  context 'An attachment with S3 storage and bucket defined as a Proc' do
    before do
      rebuild_model storage:        :s3_v2,
                    ...
                    s3_credentials: { not: :important }, s3_region: 'us-west-1'
    end
  ...
  context 'An attachment with S3 storage and S3 credentials in an unsupported manor' do
    before do
      rebuild_model storage: :s3_v2, bucket: 'testing', s3_credentials: ['unsupported']
      @dummy = Dummy.new
    end
  ...
  context 'An attachment with S3 storage and S3 credentials not supplied' do
    before do
      rebuild_model storage: :s3_v2, bucket: 'testing'
      @dummy = Dummy.new
    end
        ...
        context 'and saved' do
          before do
            ...
            [:thumb, :original].each do |style|
              ...
              expected_options = { :content_type => 'image/png', :acl => 'public-read' }
              expected_options.merge!(:storage_class => 'REDUCED_REDUNDANCY') if style == :thumb
              object.expects(:upload_file).with(anything, expected_options)
            end
            @dummy.save
          end
      ...
      context 'when assigned' do
        ...
        context 'and saved' do
          before do
            ...
            [:thumb, :original].each do |style|
              @dummy.avatar.stubs(:s3_object).with(style).returns(object)
              object.expects(:upload_file).with(anything, :content_type => 'image/png',
                                                :acl                    => 'public-read',
                                                :storage_class          => 'REDUCED_REDUNDANCY')
            end
            @dummy.save
  ...
  context 'with S3 credentials supplied as Pathname' do
    before do
      ...
      rails_env('test') do
        rebuild_model storage:        :s3_v2,
                      s3_credentials: Pathname.new(fixture_file('s3.yml')), s3_region: 'us-east-1'
    ...
    it 'parses the credentials' do
      ...
      assert_equal 'pathname_key', @dummy.avatar.s3_client.config.access_key_id
      assert_equal 'pathname_secret', @dummy.avatar.s3_client.config.secret_access_key
    end
  end
  ...
  context 'with S3 credentials in a YAML file' do
    before do
      ...
      rails_env('test') do
        rebuild_model storage:        :s3_v2,
                      s3_credentials: File.new(fixture_file('s3v2.yml'))
    ...
    it 'runs the file through ERB' do
      ...
      assert_equal 'env_key', @dummy.avatar.s3_client.config.access_key_id
      assert_equal 'env_secret', @dummy.avatar.s3_client.config.secret_access_key
    end
  end
